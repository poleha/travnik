...
best_posts = register.inclusion_tag('main/widgets/_posts_portlet.html')(
    super_tags.best_posts_maker(models.Recipe, days=30))
...
def breadcrumbs(context):
    ...
    if url_name in ['post-detail-alias', 'post-detail-alias-comment', 'post-detail-pk', 'post-detail-pk-comment']:
        ...
        breadcrumbs_list.append(
            Breadcrumb(title='Профиль пользователя {0}'.format(user), href=reverse_lazy('user-profile'),
                       class_name='active'))

    elif url_name == 'user-detail':
        user = context['current_user']
        breadcrumbs_list.append(Breadcrumb(title='Информация о пользователе {0}'.format(user),
                                           href=reverse_lazy('user-detail', kwargs={'pk': user.pk}),
                                           class_name='active'))
...
def metatags(context):
    ...
    metatags_dict[
        'description'] = 'Сайт о лекарственных растениях и обо всем, что с ними связано. Рецепты с лекарственными растениями, отзывы.'
    ...
    if url_name == 'main-page':
        ...
        if obj.is_plant:
            ...
            metatags_dict['title'] = '{0} | Medavi.ru'.format(str(obj))
            metatags_dict['keywords'] = "{0}, рецепт, отзывы, {1}".format(obj.title, obj.title, ', '.join(
                obj.plants.get_available().values_list('title', flat=True)))
            metatags_dict['description'] = "Рецепт {0} с использованием лекарственных растений {1}.".format(obj.title,
                                                                                                            ', '.join(
                                                                                                                obj.plants.get_available().values_list(
                                                                                                                    'title',
                                                                                                                    flat=True)))
